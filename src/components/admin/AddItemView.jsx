...
import { 
  ...
  Search, Save, X // TAMBAHKAN: Ikon X untuk tombol hapus
} from 'lucide-react';
        ...
        <div className="p-10 space-y-8">
                ...
                <div className="absolute z-50 w-full mt-2 bg-white rounded-[13px] shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                  <div className="max-h-48 overflow-y-auto custom-scrollbar p-1.5">
                    {filteredSuggestions.map((cat, index) => (
                      <button key={index} type="button" onClick={() => { setFormData({...formData, category: cat}); setShowSuggestions(false); }} className="w-full text-left px-4 py-2.5 rounded-[13px] text-[11px] font-bold text-slate-600 hover:bg-red-50 hover:text-red-600 transition-colors">{cat}</button>
                    ))}
                  </div>
                </div>
              )}
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2 p-1 bg-slate-100 rounded-[13px] w-fit">
              <button type="button" onClick={() => setImageMode('upload')} className={`px-4 py-2 rounded-[13px] text-[10px] font-bold transition-all ${imageMode === 'upload' ? 'bg-white text-red-600 shadow-sm' : 'text-slate-400'}`}>Upload</button>
              <button type="button" onClick={() => setImageMode('url')} className={`px-4 py-2 rounded-[13px] text-[10px] font-bold transition-all ${imageMode === 'url' ? 'bg-white text-red-600 shadow-sm' : 'text-slate-400'}`}>Link URL</button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center bg-slate-50 p-8 rounded-[13px] border-2 border-dashed border-slate-200">
              <div className="md:col-span-3 flex justify-center">
                {/* --- REVISI: TOMBOL X UNTUK HAPUS GAMBAR --- */}
                  ...
                  {formData.image ? (
                    <>
                      ...
                      <button 
                        ...
                        className="absolute top-1 right-1 p-1 bg-red-600 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-md hover:bg-red-700 active:scale-90"
                      >
                        <X className="w-3 h-3" />
                      </button>
                    </>
              ...
              <div className="md:col-span-9 w-full">
                {imageMode === 'upload' ? (
                  <div className="relative group">
                    ...
                    <div className="w-full py-10 border-2 border-dashed border-slate-300 rounded-[13px] flex flex-col items-center justify-center gap-2 bg-white group-hover:border-red-400 transition-all">
          ...
          <button type="submit" disabled={isSubmitting} className="w-full py-5 bg-gradient-to-r from-red-600 to-[#4a0404] text-white font-bold rounded-[13px] shadow-xl hover:shadow-red-200 active:scale-[0.95] transition-all flex items-center justify-center gap-3 disabled:opacity-70 mt-4">
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-[13px] shadow-2xl w-full max-w-sm overflow-hidden animate-modal-pop text-center p-10 border border-emerald-50">
            <div className="w-20 h-20 bg-emerald-50 rounded-[13px] flex items-center justify-center mx-auto mb-6 shadow-inner"><CheckCircle2 className="w-10 h-10 text-emerald-600 animate-bounce" /></div>
            ...
            <button onClick={() => { setShowSuccessModal(false); onCancel(); }} className="w-full py-4 bg-emerald-600 text-white font-bold rounded-[13px] shadow-lg hover:bg-emerald-700 transition-all active:scale-95">Selesai</button>
          </div>
        </div>
      )}
    </div>
