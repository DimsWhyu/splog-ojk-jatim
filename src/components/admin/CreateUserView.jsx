...
const CreateUserView = ({ validUsers, WEB_APP_URL, fetchAllData }) => {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setMsg({ type: 'success', text: `Akun ${formData.name} berhasil didaftarkan!` });
  ...
  const getInputStyle = (field) => {
    const base =
      'w-full rounded-xl px-4 py-3 text-sm font-semibold outline-none transition-all border shadow-sm';
    return errors[field]
    ...
    <div className="max-w-3xl animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="mb-8">
        ...
        <p className="text-slate-400 font-medium text-sm italic mt-1">
          Daftarkan akun pegawai OJK Jawa Timur secara aman dan terintegrasi.
        </p>
      </div>
      ...
      >
        ...
        {msg.text && (
          <div
            className={`p-4 rounded-xl flex items-center gap-3 text-xs font-bold border animate-in fade-in zoom-in-95 ${
              msg.type === 'error'
      ...
      <div className="mt-6 px-2">
        <p className="text-[11px] text-slate-400 font-medium italic">
          *Seluruh pendaftaran akun akan tercatat secara otomatis di Database Logistik OJK Provinsi
          Jawa Timur.
        </p>
      </div>
    </div>
